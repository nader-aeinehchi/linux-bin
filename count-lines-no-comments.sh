...
for FILE in "${FILES_ARRAY[@]}"; do
    # Filter out lines that are considered entirely comments or are blank.
    # This regex attempts to exclude:
    # - Lines that are completely blank (possibly with whitespace): '^\s*$'
    # - Lines that start with optional whitespace and are entirely a '//' comment: '^\s*//.*$'
    # - Lines that start with optional whitespace and are entirely a '/*' comment: '^\s*/\*.*$'
    # - Lines that start with optional whitespace and are entirely a '*' (for multi-line block): '^\s*\*.*$'
    # - Lines that start with optional whitespace and are entirely a '*/' comment: '^\s*\*/.*$'
    # Lines with code AND a trailing comment (e.g., 'int x = 0; // comment') will now be counted.
    # '2>/dev/null' suppresses errors if a file cannot be read, and '|| echo 0' ensures LINE_COUNT is always a number.
